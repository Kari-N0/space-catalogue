...
| 002 | Blue Moon Mk2 | 3 | **draft v003 — art pass pending** | `assets/mk2/mk2_v003.blend` (+ `BRIEF.md`) | v003 = detail+materials pass from Kari's Gemini 6-view sheet: switchback stairs, catwalk, two-tone legs, procedural PBR (paint/MLI/alu/blue), lettering + flag; 3× BE-7 kept per NASA over the sheet's 7 bells. Also on disk: v002 (hand model, official render), mk2_ai_v001 (TRELLIS evaluation, NOT CLEARED for shipping). Validator 8/8; turnarounds `D:\renders\lunar-base\assets\mk2_v003\` |
...
| T02 | Catalogue Tools add-on — module 1: capture panel (N-panel UI over T01) | ADDON.md + CAPTURE.md §8 | **v0.1 built 2026-07-14 — Kari click-test iterations running** | `pipeline/blender/addons/catalogue_tools/` | Thin UI only, no ML imports, Execute = headless WSL job + status-file polling; dev-registered live over MCP (hot-reload loop), extension packaging when the feature set settles |
