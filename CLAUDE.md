...
- **Concept JSON camera block — ownership split (2026-07-24):** envelope-owned (generated, never hand-edit; re-export from Blender): `look_at_m`, `distance_m.min/max`, `angle_up_down_deg.min/max`, `angle_around_deg.min/max`, `object_envelopes.*`. Kari-owned (hand-edited freely): all `*.start` opening-shot fields, `controls.*`, `move_limit_m`, `zoom_fov_deg`, `clip_near_m/clip_far_m`, and all `pins`. The parser console-warns on unrecognized/clamped camera keys (silent-ignore banned); `?debug=camera` shows the loaded values live; full schema in content/concepts/README.md.
- **Signup responses stay generic (Kari, 2026-08-04 — deliberate, enumeration protection):** the concept-page signup form shows the same "if this address isn't already on the list, a confirmation email is on its way" message and stays locked for EVERY Buttondown server response (success, already-subscribed, flagged address); only network errors re-enable the form. Never echo list membership back to the visitor, never branch on Buttondown's response body. (Empirical shapes 2026-08-04: fresh clean address = 302→hosted page, both hops CORS-readable; duplicate and flagged-domain = identical 400 + Turnstile CAPTCHA page — indistinguishable anyway. Resubscribe-after-unsubscribe is suppressed by Buttondown's suppression list — accepted, LAUNCH.md footnote.)
