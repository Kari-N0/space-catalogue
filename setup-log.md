...
## TRELLIS.2 unblock check — DINOv3 / Comfy-Org research (read-only, no installs) — 2026-07-12

**Task (Kari):** check whether the Comfy-Org TRELLIS.2 repack + current DINOv3 license terms unblock TRELLIS.2 without the pending Meta grant. Research only; GATE before any download.

**Method:** 10-agent workflow (run wf_2b7f8ed1-839) — 5 recon (license text, HF repo metadata, ComfyUI support, Meta access path, local state) + 5 adversarial verifiers (verbatim quote audit, redistribution refuter, EU/commercial refuter, gating and support re-checks). Primary sources only; every quote below survived the quote audit against independently re-fetched texts.

**Finding 1 — DINOv3 license, full text obtained (variants identified):**
- Canonical weights text: `facebookresearch/dinov3` LICENSE.md, "Last Updated: August 19, 2025" (sha256 `25d122eb8f5b880fd23c736fb6ea8018ee45c12237e00b8a86d14c653904999e`; byte-identical copy ships inside the HF model repos).
- The ai.meta.com page that the HF gate's `license_link` targets still serves an "August 14, 2025" variant whose §1.b.i **adds**: *"(B) prominently display 'Built with DINOv3' on a related website, user interface, blogpost, about page, or product documentation"* when distributing DINO Materials or derivatives. That clause never appeared in any repo/HF text. Conservative compliance: honor it if we ever redistribute weights or fine-tunes.
- Confirms and sharpens the 2026-07-09 reading: grant §1.a is "non-exclusive, worldwide, non-transferable and royalty-free … use, reproduce, distribute, copy, create derivative works of, and make modifications". Verified ABSENT by full-text term counts: commercial/research-only limits, MAU or revenue thresholds, territorial or EU exclusion, any Acceptable Use Policy, any restriction on training other models. Present: Trade Controls + ITAR/military/nuclear/espionage/weapons end-use bans (§1.b.iii–v), reverse-engineering ban (§1.b.iv), publication acknowledgment (§1.b.ii), you own your derivatives (§5.a), uncapped indemnity + litigation auto-termination (§5.b), termination on any breach → delete and cease (§6), unilateral modification with immediate effect (§8 — hash the accepted text in provenance), California law/forum (§7). **Outputs are treated separately from "derivative works" → publishing generated 3D assets does not trigger the redistribution conditions at all.**

**Finding 2 — redistribution terms (§1.b.i):** third-party redistribution is permitted **only** "under the terms of this Agreement" and "you shall provide a copy of this Agreement with any such DINO Materials" (plus the Built-with notice in the ai.meta.com variant).

**Finding 3 — Comfy-Org/TRELLIS.2 (HF) fails those conditions:** repo is ungated (`gated: false`), blanket-tagged `license: mit`, and contains `clip_vision/dino_v3_vit_l.safetensors` (1.21 GB FP32) — safetensors-header inspection shows HF-transformers DINOv3ViT ViT-L/16 tensors, i.e. **verbatim DINO Materials** from exactly the gated repo TRELLIS.2 needs (`facebook/dinov3-vitl16-pretrain-lvd1689m`; Microsoft ships no encoder and loads it frozen at runtime, so no independent-provenance argument exists). No LICENSE/NOTICE file, no DINOv3 mention anywhere in the repo. Adversarial verdict: **REFUTED** — the repack as performed breaches §1.b.i (distributed not-under-the-Agreement, no copy of it, relabeled MIT), and for an applicant Meta already rejected, license-formation-by-conduct via a third-party copy is genuinely undetermined — while *using* the weights would certainly bind Vidro to the burdens (uncapped indemnity, CA forum, delete-on-termination). Contrast: timm / onnx-community / camenduru also redistribute DINOv3 ungated but carry the `dinov3-license` tag + link; Comfy-Org's MIT relabel is the outlier.

**Finding 4 — ComfyUI native TRELLIS.2: not shipped.** Staging only: PR #12183 "Add Trellis 2 Support" merged 2026-05-21 into branch `trellis2` (not master); successor PR #14718 (kijai, → master) still open 2026-07-12; absent from all releases ≤ v0.27.0; no blog/docs. The design is clean for our rules when it lands: user-supplied mask (no RMBG anywhere — grepped both branches), mesh + PBR bake only (no gaussians), no extra pip/CUDA deps (CuMesh/xatlas reimplemented in torch/scipy).

**Finding 5 — Meta-direct path intel (pending application):** Meta's FAQ frames rejections purely as sanctioned-jurisdiction screening (the gate form has no use-case field; it does capture IP geolocation — submit without VPN). HF-rejected → Meta-direct-approved is a reported working path (dinov3 issues #24, #322). Direct-download links **expire in ~2 days** — convert immediately on receipt; re-requesting fresh links is normal (#202). HF re-application workaround: delete the rejected entry at huggingface.co/settings/gated-repos and resubmit. Observed turnarounds: 15 min–3 weeks.

**Local-state extras (activation-time TODO):** `trellis2_texturing.py:86` instantiates the bg remover without the RMBG guard that `trellis2_image_to_3d.py:106-109` has (currently harmless — local rembg module only exports `RembgU2Net`); add the guard when activating. Banned-dep scan of ~/apps/TRELLIS.2 clean (no diff-gaussian/graphdeco/briaai code paths). ComfyUI at v0.27.0-36-g04a30fb3, torch 2.13.0+cu130; trellis2 env torch 2.6.0+cu124 intact; 832 GB free on ext4.
